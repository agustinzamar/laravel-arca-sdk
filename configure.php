...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i .vendor\ | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package AgustinZamar skeleton migration_table_name vendor_name vendor_slug author@domain.com"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i ":author|:vendor|:package|AgustinZamar|skeleton|migration_table_name|vendor_name|vendor_slug|author@domain.com" --exclude-dir=vendor ./* ./.github/* | grep -v ' . basename(__FILE__)));
}
...
$AgustinZamar = ask('Vendor name', $guessGitHubVendorInfo[0]);
$vendorUsername = ask('Vendor username', $guessGitHubVendorInfo[1] ?? slugify($AgustinZamar));
$vendorSlug = slugify($vendorUsername);

$AgustinZamarspace = str_replace('-', '', ucwords($AgustinZamar));
$AgustinZamarspace = ask('Vendor namespace', $AgustinZamarspace);
...
writeln("Vendor     : {$AgustinZamar} ({$vendorSlug})");
writeln("Package    : {$packageSlug} <{$description}>");
writeln("Namespace  : {$AgustinZamarspace}\\{$className}");
...
foreach ($files as $file) {
    replace_in_file($file, [
      ...
      ':vendor_name' => $AgustinZamar,
      'agustinzamar' => $vendorSlug,
      'AgustinZamar' => $AgustinZamarspace,
      'Laravel ARCA SDK' => $packageName,
      'laravel-arca-sdk' => $packageSlug,
      'laravel-arca-sdk_without_prefix' => $packageSlugWithoutPrefix,
      'Wsaa' => $className,
    ...
    match (true) {
        str_contains($file, determineSeparator('src/Wsaa.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/LaravelArcaSdkServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/Facades/Wsaa.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        ...
        str_contains($file, determineSeparator('config/laravel-arca-sdk.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
